...
public class JxtaLoaderModuleFactory<T extends Module> implements IJxtaModuleFactory<T>{
     ...
     * <p/>
     */
    private static JxtaLoaderModuleFactory<Module> root;

    private final static transient Logger LOG = Logging.getLogger( JxtaLoaderModuleFactory.class.getName());

    private JxtaLoaderModuleFactory<? extends Module> parent = null;
	...
	protected JxtaLoaderModuleFactory( ClassLoader loader) {
	 ...
	 * @param loader
	 */
	private JxtaLoaderModuleFactory( IJxtaLoader loader) {
		this.loader = loader;
		builders = new ArrayList<IModuleBuilder<T>>();
	...
	private JxtaLoaderModuleFactory( IJxtaLoader loader, JxtaLoaderModuleFactory<? extends Module> parent, PeerGroup peergroup ) {
		this.loader = loader;
		this.parent = parent;
		this.verifier = getVerifier( (IJxtaModuleFactory<T>) parent, peergroup );
		managers = new HashMap<PeerGroup, IModuleManager<? extends Module>>();
		builders = new ArrayList<IModuleBuilder<T>>();
		this.started = false;
		lock = new ReentrantLock();
	...
	public static ClassLoader getClassLoader( PeerGroup peergroup){
		JxtaLoaderModuleFactory<? extends Module> manager = (JxtaLoaderModuleFactory<? extends Module>) managers.get( peergroup );
		if( manager == null )
	...
	public static IModuleManager<Module> createModuleManager( PeerGroup peergroup, PeerGroupAdvertisement peerGroupAdvertisement ){
        ...
        if (null == parentGroup) {
        	...
        	JxtaLoaderModuleFactory<? extends Module> pm = (JxtaLoaderModuleFactory<? extends Module>) managers.get( parentGroup ); 
            if( pm == null )
            	pm = root;
            ...
            if (cfgDoc != null) {
                ...
                if (pgca.isFlagSet(PeerGroupConfigFlag.SHUNT_PARENT_CLASSLOADER)) {
                    ...
                    // definitions).
                    upLoader = (IJxtaLoader)JxtaLoaderModuleFactory.class.getClassLoader();
                }
            }

            Logging.logCheckedDebug(LOG, "Setting up group loader -> ", upLoader);
            manager = new JxtaLoaderModuleFactory<Module>(upLoader, pm, peergroup );
	 ...
	 * @return
	 */
	private ModuleVerifier<T> getVerifier( IJxtaModuleFactory<T> manager, PeerGroup peergroup ){
	 ...
	 * @return
	 */
	public static JxtaLoaderModuleFactory<Module> getRoot(){
	 ...
	 * @return
	 */
	public static JxtaLoaderModuleFactory<Module> getRoot( Class<?> clzz ){
	 ...
	 * @return
	 */
	public static JxtaLoaderModuleFactory<Module> getRoot( Class<?> clzz, boolean forceNew ){
		if(( root == null ) || ( forceNew )){
			IJxtaLoader staticLoader = new RefJxtaLoader( new URL[0], clzz.getClassLoader(), COMP_EQ);
			root = new JxtaLoaderModuleFactory<Module>( staticLoader );
		}
		return root;
	}
